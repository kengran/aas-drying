...
import nano_43rh as targetFile
...
data_files_1=[targetFile.Readfile()[1][i] for i in [0,2,10,15]]
name_of_the_plot=['%s h'%(array(targetFile.Data()[0][i])) for i in [0,2,10,15]]
...
'''select which PDF to plot here'''
[ax.plot(interm2_syn[i][0],np.array(interm2_syn[i][1])*normalization_factor[i], alpha=1,label='%s'%(name_of_the_plot[i])) for i in range (shape(data_files_1)[0])]
...
plt.subplots_adjust(left=0.1, bottom=0.11, right=0.9, top=0.9)
